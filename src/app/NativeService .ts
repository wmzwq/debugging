...
export class NativeService extends BaseUI {
  ...
  detectionUpgrade() {
    ...
    this.appVersion.getVersionNumber().then(res => {
      var _that = this;
      var url = "http://122.228.89.215:8887/CheckAndAcceptAPI/VersionUpgrade";
      this.httpservice.post(url, JSON.stringify({
        "data": "{'pagination':{rows:50,page: 1,sidx:'F_Version',sord:'DESC'},'queryJson':\"{'F_AppType':'3'}\"} "

      })).then(req => {
        try {
          console.log("banben", req)
          var appVersion = req.data.rows[0].F_Version
          var downLoadUrl = req.data.rows[0].F_DownLoadURL
  ...
  downloadApp() {
    if (this.isAndroid()) {
      ...
      const apk = this.file.externalApplicationStorageDirectory + '电梯调试app.apk'; //apk保存在手机里的目录
